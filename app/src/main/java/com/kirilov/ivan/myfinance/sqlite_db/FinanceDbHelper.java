package com.kirilov.ivan.myfinance.sqlite_db;
...
public class FinanceDbHelper extends SQLiteOpenHelper {
    ...
    public long createAcc(Wallet wallet){
        SQLiteDatabase db=this.getWritableDatabase();

        ContentValues contentValues = new ContentValues();
        contentValues.put(FinanceContract.AccountsEntry.ACC_NAME, wallet.getWalletName());

        return db.insert(FinanceContract.AccountsEntry.TABLE_NAME, null, contentValues);
    }

    // find ACC by ID and return it
    public Wallet getAcc(long id){
        SQLiteDatabase db = this.getReadableDatabase();
        ...
        wallet.setWalletId(cursor.getLong(cursor.getColumnIndex(FinanceContract.AccountsEntry.ACC_ID)));
        wallet.setWalletName(cursor.getString(cursor.getColumnIndex(FinanceContract.AccountsEntry.ACC_NAME)));
    ...
    public List<Wallet> getAllAccs(){
        SQLiteDatabase db = this.getReadableDatabase();
        List<Wallet> accs = new ArrayList<>();
    ...
    public int updateAcc(Wallet wallet){
        SQLiteDatabase db = this.getWritableDatabase();

        ContentValues contentValues = new ContentValues();
        contentValues.put(FinanceContract.AccountsEntry.ACC_ID, wallet.getWalletId());
        contentValues.put(FinanceContract.AccountsEntry.ACC_NAME, wallet.getWalletName());

        return db.update(FinanceContract.AccountsEntry.TABLE_NAME,
                contentValues,
                FinanceContract.AccountsEntry.ACC_ID + " = ?",
                new String[]{String.valueOf(wallet.getWalletId())});
    }
    ...
    public long createTransaction(Transaction transaction){
        ...
        contentValues.put(FinanceContract.TransactionEntry.TR_DATE,transaction.getTrDate());
        contentValues.put(FinanceContract.TransactionEntry.TR_ACCOUNT,transaction.getTrWallet());
        contentValues.put(FinanceContract.TransactionEntry.TR_CATEGORY,transaction.getTrCat());
        contentValues.put(FinanceContract.TransactionEntry.TR_AMOUNT,transaction.getTrAmount());
        contentValues.put(FinanceContract.TransactionEntry.TR_CURRENCY,transaction.getTrCurrency());
        contentValues.put(FinanceContract.TransactionEntry.TR_DESCR, transaction.getTrDesc());
