...
/**
 * Created by Ivan on 06-Jun-16.
 */
public class FirebaseExpenseActivity extends BaseActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.firebase_activity_expense);
        ...
        toolbar = (Toolbar) findViewById(R.id.firebase_expense_toolbar);
        ...
        amountText = (TextInputEditText) findViewById(R.id.firebase_expense_amount);
        descriptionText = (TextInputEditText) findViewById(R.id.firebase_expense_description);

        if (extraTransaction.getTrAmount() != 0d){
            isNewTransaction = false;

            Button btnSave = (Button) findViewById(R.id.firebase_expense_btn_add);
            ...
            Button btnDelete = (Button) findViewById(R.id.firebase_expense_btn_delete);
            btnDelete.setVisibility(View.VISIBLE);

            // first remove the - => then set text
        ...
        currencyText = (TextView) findViewById(R.id.firebase_expense_currency);
    ...
    public void setupWalletsRecycleView(){
        walletsRecycleView = (RecyclerView) findViewById(R.id.firebase_expense_wallet_recycle);
        ...
        query.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                if (dataSnapshot.hasChildren()){
                    ...
                    Toast.makeText(context, "Error.", Toast.LENGTH_SHORT).show();
                    FirebaseExpenseActivity.this.finish();
                }
    ...
    public void setupCategoriesRecycleView(){
        categoriesRecyclerView = (RecyclerView) findViewById(R.id.firebase_expense_category_recycle);

        categoriesRecyclerView.setHasFixedSize(true);

        categoriesGridLayoutManager = new GridLayoutManager(context, 2, GridLayoutManager.HORIZONTAL, false);
    ...
    private void setupDatePickerDialog(){
        dateText = (TextView) findViewById(R.id.firebase_expense_date);
    ...
    public void btnAddEditExpense(View view){

        extraTransaction.setTrWallet(walletId);
        extraTransaction.setTrCat(categoryId);

        try {
            // multiply by -1 because its EXPENSE
            extraTransaction.setTrAmount(-1 * Double.parseDouble(amountText.getText().toString()));
    ...
    public void btnDeleteExpense(View view){
    ...
    private void addExpense(){
        // userTrans/EMAIL/WALLET_ID/1/CAT_ID/BEGIN_OF_MONTH/TRANSACTION_ID
...
//---   ReCalculate transactions WALLET income and balance
        ...
        Utilities.addExpenseToWallet(extraTransaction.getTrWallet(), extraTransaction, context);

//---   ReCalculate TOTAL BALANCE's income and balance
        // get reference to -> userWalletsHistory/EMAIL/WALLET_ID/BEGIN_OF_MONTH
        Utilities.addExpenseToWallet(0L, extraTransaction, context);

        finish();
    }

    private void deleteExpense(){
        Log.d("BUGHUNT: ", "deleteIncome: START");
        // userTrans/EMAIL/WALLET_ID/0/CAT_ID/BEGIN_OF_MONTH/TRANSACTION_ID
        DatabaseReference referenceWallet = firebaseDatabase.getReference(Constants.FIREBASE_LOCATION_USER_TRANSACTIONS + "/"
                + PreferenceManager.getDefaultSharedPreferences(context).getString(Constants.KEY_PREF_EMAIL_PARSED, null) + "/"
...
//---    now ReCalculate transactions WALLET expense and balance
        //To do so -> multiply TR_AMOUNT by -1
        transToDelete.setTrAmount(-1 * transToDelete.getTrAmount());
        Utilities.addExpenseToWallet(transToDelete.getTrWallet(), transToDelete, context);

//---   now ReCalculate TOTAL BALANCE's income and balance
        Utilities.addExpenseToWallet(0L, transToDelete, context);
...
//---    add the transaction in TOTAL BALANCE wallet
                ...
                transToDelete.setTrAmount(-1 * transToDelete.getTrAmount());
                Utilities.editExpenseWallet(transToDelete.getTrWallet(), transToDelete, extraTransaction, context);

                Utilities.editExpenseWallet(0L, transToDelete, extraTransaction, context);
