...
public class AddExpenseActivity extends BaseActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        Toolbar toolbar = (Toolbar) findViewById(R.id.add_expense_toolbar);
...
//        getSupportActionBar().setHomeButtonEnabled(true);
    ...
    public void addDebitTransaction(View view){
        ...
        transaction.setTrDate(chosenDateInMs);
        transaction.setTrWallet((int) spinnerAcc.getSelectedItemId());
        transaction.setTrCat((int) spinnerCategory.getSelectedItemId());
        try {
            transaction.setTrAmount(-1 * Double.parseDouble(amountText.getText().toString()));
        }catch (NumberFormatException e){
            NO_AMOUNT = true;
            Toast.makeText(getApplicationContext(), "Please enter amount.",Toast.LENGTH_SHORT).show();
        }
        transaction.setTrCurrency(PreferenceManager.getDefaultSharedPreferences(context).getString(Constants.KEY_PREF_CURRENCY,"BGN"));
        transaction.setTrDesc(descText.getText().toString());
        ...
        if(checkTransaction(transaction, NO_AMOUNT)) {
            ...
            // debugging
            Log.d("FINANCE DB:", " " + transaction.getTrDate() + " " + transaction.getTrWallet() + " " + transaction.getTrCat() +
                    " " + transaction.getTrAmount() + " " + transaction.getTrCurrency() + " " + transaction.getTrDesc());


            // FIREBASE
            DatabaseReference expenseReference = firebaseDatabase.getReference(Constants.FIREBASE_LOCATION_USER_TRANSACTIONS + "/"
                                                                 + firebaseAuth.getCurrentUser().getEmail().replace('.',',') + "/"
                                                                 + transaction.getTrWallet() + "/"
                                                                 + transaction.getTrCat() + "/"
                                                                 + transaction.getTrDate());
                Log.d("FIREBASE", "ADD EXPENSE" + expenseReference.toString());
            expenseReference.setValue(transaction);
