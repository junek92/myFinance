...
public class BarChartActivity extends BaseActivity{
    ...
    public List<String> getYearsForSpinner(){
        ...
        if (!transList.isEmpty()){
            ...
            long minTime = transList.get(0).getTrDate();
            long maxTime = transList.get(0).getTrDate();
            for (int i = 0; i < transList.size(); i++) {

                if (minTime >= transList.get(i).getTrDate()) {
                    minTime = transList.get(i).getTrDate();
                }

                if (maxTime <= transList.get(i).getTrDate()) {
                    maxTime = transList.get(i).getTrDate();
                }
            }
            ...
            barChart.setOnChartValueSelectedListener(new OnChartValueSelectedListener() {
                ...
                public void onValueSelected(Entry e, int dataSetIndex, Highlight h) {
                    ...
                    if (e.getVal() > 0f){
                        ...
                        intent.putExtra(MainActivity.KEY_DATE, monthToPass);
                        intent.putExtra(MainActivity.KEY_CAT_ID, spinnerCategory.getSelectedItemId());
